...
[2. 파이프라인](#2-파이프라인) <br>
  ...
  - [phase 5: 모바일 웹 앱(PWA) 구축](#phase_5:_모바일_웹_앱(PWA)_구축) 
...
### $\color{blue}{\text{Phase 0: 데이터 정제}}$        
 ...
 - Step 0. 데이터셋 병합 merge_dataset.py, 데이터 재분배 resplit_dataset.py, 포즈 데이터 삭제 remove workout_pose.py
...
- 레터박싱(Letterboxing) 처리: 남는 여백 공간은 YOLO 학습 기본 패딩 색상(RGB: 114, 114, 114)으로 채워 640x640 정방형 이미지를 자동 생성하여 assets/images/ 디렉토리에 저장.

- $\color{blue}{\text{﻿Step 4-3. ﻿﻿﻿﻿﻿Gradio 프레임워크 기반 웹 UI 추론 구현 (app.py)}}$

> ﻿YOLO 모델과 JSON 메타데이터를 결합하여 사용자 피드백을 실시간으로 제공하는 인터페이스를 구축합니다.
...
### <span style="color:blue">Phase 5: 모바일 웹 앱(PWA) 구축 및 배포</span>

- <span style="color:blue">Step 5-1. 모델 배포 및 서버 전략 수립</span>

- Windows/WSL 환경 제약(Xcode 불가)에 따라 CoreML 네이티브 앱 대신 FastAPI/Gradio + PWA 하이브리드 전략을 채택했습니다. 기기 부하를 줄이기 위해 서버 추론 방식을 병행 검토합니다.

<img width="425" height="190" alt="image" src="https://github.com/user-attachments/assets/50a4ecd0-172a-4309-8ade-eb2db99764bf" />

- <span style="color:blue">Step 5-2. 백엔드 (FastAPI & Gradio)</span>
- server.py(REST API) 및 app.py(통합 UI) 병행 구축
- YOLO26 추론 엔드포인트 + 메타데이터 DB(JSON) 연동
- 응답 포맷: 포트 충돌 및 무한 로딩 버그가 해결된 안정적인 JSON (클래스 ID, confidence, bbox, 메타데이터) 반환

- <span style="color:blue">Step 5-3. 프론트엔드 및 모바일 배포 (PWA)</span>
- Fast API + PWA로 기종 제약 없는 온디바이스 UX 구축
- 카메라 촬영 → ngrok 터널링을 통한 서버 API 호출 및 실시간 렌더링
...
> 같은 주제를 선정한 정영석님의 small.pt를 학습에 활용 (7개 데이터셋 중 1개만(514장, 12개 클래스)) (해당 학습은 28개 클래스, ~38,000장)
